...
export default function ProCard({ pro, index = 0 }: ProCardProps) {
  ...
  const isClaimed    = pro.is_claimed !== false  // default true — false only when explicitly unclaimed
  const licenseNum   = (pro as any).license_number || null
  ...
  if (!isClaimed) {
    return (
      <Link
        href={`/claim/${pro.id}`}
        className="group block bg-white rounded-xl p-5 hover:-translate-y-0.5 transition-all duration-200 relative"
        style={{ border: '1px solid #E8E2D9', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', animationDelay: `${index * 40}ms` }}
      >
        {/* Unclaimed banner */}
        <div className="absolute top-0 left-0 right-0 rounded-t-xl px-4 py-1.5 flex items-center justify-between"
          style={{ background: '#FFF8F0', borderBottom: '1px solid #F5E6D0' }}>
          <span className="text-xs font-semibold" style={{ color: '#92400E' }}>Unclaimed profile</span>
          <span className="text-xs font-semibold" style={{ color: '#0D9488' }}>Claim free →</span>
        </div>

        {/* Header — nudged down for banner */}
        <div className="flex gap-3 items-start mt-6 mb-3">
          <div className="relative flex-shrink-0">
            <div
              className="w-11 h-11 rounded-full flex items-center justify-center text-sm font-serif flex-shrink-0 opacity-60"
              style={{ background: bg, color: fg }}>
              {initials(pro.full_name)}
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-gray-700 truncate text-sm leading-snug">{pro.full_name}</div>
        ...
        {/* License number — key trust signal */}
        {licenseNum && (
          <div className="flex items-center gap-1.5 mb-3 px-3 py-2 rounded-lg"
            ...
            <svg width="12" height="12" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
              <path d="M16 2L4 7V16C4 22.6 9.4 28.4 16 30C22.6 28.4 28 22.6 28 16V7L16 2Z" fill="#0D9488" opacity="0.7"/>
              <path d="M11 16l3 3 7-7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span className="text-xs font-semibold" style={{ color: '#0D7377' }}>DBPR License</span>
            <span className="text-xs font-mono font-semibold text-gray-700 ml-auto">{licenseNum}</span>
          </div>
        )}
        ...
        <div className="flex text-center mb-3">
          <div className="flex-1">
            <div className="text-xs font-semibold text-gray-400">Not yet claimed</div>
            <div className="text-xs text-gray-400 mt-0.5">No reviews yet</div>
          </div>
